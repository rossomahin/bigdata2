...
# Разделяем данные на обучающую и тестовую выборки с 30% данных для теста.
X_train, X_test, y_train, y_test = train_test_split(X, y, test_size=0.3, random_state=42)

# 3. Обучение модели и поиск оптимального C
# Перебираем различные значения гиперпараметра C для нахождения оптимального.
C_values = [0.2, 0.4, 0.6, 0.8, 1, 1.2, 1.4, 1.6, 1.8, 2.0]
best_C = C_values[0]  # Инициализация наилучшего значения C
best_accuracy = 0  # Инициализация лучшей точности
...
for C in C_values:
    ...
    # Обновляем наилучшее значение C, если точность улучшилась
    if accuracy > best_accuracy:
        best_accuracy = accuracy
        best_C = C

# Выводим оптимальное значение C и среднюю ошибку
